...
RSpec.describe Key do
  ...
  it 'can create offsets' do
    expect(@key.create_offsets).to be_a(String)
    expect(@key.create_offsets.length).to eq(4)

    allow(@key).to receive(:create_offsets).and_return('5641')
    expect(@key.create_offsets).to eq('5641')
  end

  it 'can create offsets with a date input as a numeric DDMMYY string' do
    expect(@key.create_offsets('160221')).to eq('8841')
  end
