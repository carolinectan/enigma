...
RSpec.describe Key do
  ...
  it 'can create a key' do
    expect(@key.create_key('12345')).to eq('12345')

    expect(@key.create_key('abc')).to be_a(String)
    expect((@key.create_key('abc')).length).to eq(5)
    expect(@key.create_key('abc').to_i).to be_a(Integer)
  end

  xit 'can test that it returns an array of 5 numbers' do
  ...
  xit 'can create offsets with a date input as a numeric DDMMYY string' do
    expect(@key.create_offset('160221')).to eq('8841')
  end

  xit 'can return a hash of the final shift values' do    # key = double('39927')
    # stubbed_expected = {
    #                       :a => 55,
    #                       :b => 14,
    #                       :c => 88,
